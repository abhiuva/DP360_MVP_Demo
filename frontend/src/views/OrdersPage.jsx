...
export default function OrdersPage() {
  ...
  const btnShowPayAndComplete = async (orderIds, order) => {
    try {
      ...
      if (res.status == 200) {
        const { subtotal, taxTotal, discount_total, total, orders } = res.data;
        ...
        setState({
          ...
          summaryDiscount: discount_total,
          summaryTotal: total,
  ...
  const btnPayAndComplete = async () => {
    ...
    const payload = {
      orderIds: state.completeOrderIds,
      subTotal: state.summaryNetTotal,
    ...
    console.log("Sending to /complete-and-pay-order:", payload);
    ...
    if (!payload.selectedPaymentType || isNaN(payload.selectedPaymentType)) {
      toast.error("Please select a valid payment method.");
      return;
    }

    const selectedPayment = state.paymentTypes.find(
      (p) => p.id === payload.selectedPaymentType
    );

    if (!selectedPayment) {
      toast.error("Selected payment type not valid.");
      return;
    }
    ...
    try {
      ...
      if (
        ...
        const response = await axios.post(
          `${import.meta.env.VITE_BACKEND}/orders/stripe-checkout`,
          {
            ...
            successUrl: `${window.location.origin}/orders/stripe-success`,
      ...
      if (res.status === 200) {
        ...
        if (isPrintReceipt) {
          ...
          const orderIdsText = payload.orderIds.join(", ");
          for (const o of state.summaryOrders) {
            ...
            items.forEach((i) => {
              ...
              orders.push({
                ...i,
                title: i.item_title,
                addons_ids: i?.addons?.map((a) => a.id) || [],
                variant,
              });
            });
          }

          const { customer_id, customer_type, customer_name, delivery_type } =
            state.summaryOrders[0];

          const paymentType = state.paymentTypes.find(
            (v) => v.id === payload.selectedPaymentType
          );
          ...
          setDetailsForReceiptPrint({
            ...
            storeSettings,
            printSettings,
            itemsTotal: payload.subTotal,
            taxTotal: payload.taxTotal,
            discount: payload.discount,
            payableTotal: payload.total,
            tokenNo: state.completeTokenIds,
            orderId: orderIdsText,
      ...
      const message =
        error?.response?.data?.message ||
        "Payment failed! Please check your network or data.";
      toast.error(message);
    }
  };
              ...
              >
                <div className="flex items-center flex-col md:flex-row md:justify-between text-center gap-2 pb-2">
                  ...
                  <div className="dropdown dropdown-end">
                    ...
                    >
                      {/* Button Print Receipt commended untill bug is fixed */}
                      {/* <li>
                        <button
                      ...
                      <li>
                        <button
                          ...
                          onClick={() => {
                            if (isPaid == false) {
                              toast.error(
                                "Action cannot be processed because payment has not been collected. Please use the 'Pay & Complete' option instead."
                              );
                              return;
                            }
                      ...
                      <li>
                        <button
                          ...
                          onClick={() => {
                            if (isPaid == true) {
                              toast.error(
                                "Action cannot be processed because payment has already been collected. Please use the 'Complete' option instead."
                              );
                              return;
                            }
                ...
                {orders.map((o, i) => {
                  ...
                  return (
                    <div
                      key={o.id || i}
                      className="py-2 bg-gray-50 px-2 mt-2 mb-2 rounded-xl"
                    >
      ...
      <dialog id="modal-order-summary-complete" className="modal">
        <div className="modal-box">
          ...
          <div className="my-6">
            ...
            {/* Print receipt option commended untill bug is fixed */}
            {/* <label
          ...
          <div className="grid grid-cols-3 gap-2">
            {paymentTypes.map((paymentType) => {
              return (
                <label key={paymentType.id} className="">
                  <input
                    checked={state?.selectedPaymentType === paymentType.id}
                    onChange={(e) => {
                      setState({
                        ...state,
                        selectedPaymentType: parseInt(e.target.value),
                      });
                    }}
                    type="radio"
                    name="payment_type"
            ...
            <form method="dialog" className="w-full">
              <button
                onClick={(e) => {
                  ...
                  const selectedPayment = state.paymentTypes.find(
                    (p) => p.id === state.selectedPaymentType
                  );

                  if (selectedPayment.icon === "terminal") {
