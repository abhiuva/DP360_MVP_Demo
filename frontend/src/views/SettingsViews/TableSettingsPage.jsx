...
export default function TableSettingsPage() {
  ...
  const tableOccupancyStatusRef = useRef();
  ...
  const tableOccupancyStatusUpdateRef = useRef();
  ...
  async function btnAdd() {
    ...
    const occupancyStatus = tableOccupancyStatusRef.current.value;
    ...
    if (!occupancyStatus) {
      toast.error("Please provide Occupancy Status!");
      return;
    }

    try {
      toast.loading("Please wait...");
      const res = await addNewStoreTable(
        title,
        floor,
        seatingCapacity,
        occupancyStatus
      ...
      if (res.status == 200) {
        ...
        tableOccupancyStatusRef.current.value = null;
  ...
  const btnShowUpdate = async (
    ...
    if (!id || id === "undefined") {
      toast.error("Invalid table ID passed for update.");
      return;
    }
    ...
    tableOccupancyStatusUpdateRef.current.value = occupancyStatus;
    document.getElementById("modal-update").showModal();
  };

  const btnUpdate = async () => {
    const id = parseInt(tableIdRef.current.value, 10);
    if (!id || isNaN(id)) {
      toast.error("Invalid table ID. Cannot proceed.");
      return;
    }
    ...
    const seatingCapacity = parseInt(
      tableSeatingCapacityUpdateRef.current.value,
      10
    );
    const occupancyStatus = tableOccupancyStatusUpdateRef.current.value;

    if (!title || !floor || isNaN(seatingCapacity) || !occupancyStatus) {
    ...
    try {
      ...
      const res = await updateStoreTable(
        id,
        title,
        floor,
        seatingCapacity,
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-gray-400">Status: {occupancy_status}</p>
                </div>
              ...
              {/* <button
                ...
                className="btn btn-xs h-10 bg-gray-100 text-gray-500"
              >
                <IconQrcode size={18} stroke={iconStroke} /> Download Table QR
              </button> */}
            </div>
          );
        })}
        ...
        <div className="modal-box">
          ...
          <div className="flex gap-4 w-full my-4">
            <div className="flex-1">
              ...
              <input
                ref={tableSeatingCapacityRef}
                type="number"
                min={0}
                name="capacity"
                className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light"
                placeholder="Enter Seating Capcity"
              />
            </div>
            <div className="flex-1">
              <label
              ...
              <select
                ref={tableOccupancyStatusRef}
                name="status"
                id="status"
                defaultValue=""
                className="block w-full h-9.1 border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light"
              >
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  tableFloorRef.current.value = null;
                  tableSeatingCapacityRef.current.value = null;
                  tableOccupancyStatusRef.current.value = "";
                  document.getElementById("modal-add").close(); // ✅ Close modal
                }}
              >
                Close
              </button>
        ...
        <div className="modal-box">
          ...
          <div className="flex gap-4 w-full my-4">
            <div className="flex-1">
              ...
              <input
                ref={tableSeatingCapacityUpdateRef}
                type="number"
                name="capacity"
                min={0}
                className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light"
            ...
            <div className="flex-1">
              ...
              <select
                ref={tableOccupancyStatusUpdateRef}
                name="status"
                id="status"
                className="block w-full h-9.1 border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light"
              >
