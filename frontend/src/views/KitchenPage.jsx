  ...
  return (
    <Page>
      ...
      {kitchenOrders?.length > 0 && (
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
          {kitchenOrders.map((order, index) => {
            ...
            return (
              <div
                key={index}
                className="border border-salespos-border-green-light rounded-2xl px-4 py-5 flex flex-col"
              >
                    ...
                    return (
                      <div key={index} className="flex items-center gap-2 py-2">
                        ...
                        <button
                          ...
                          onClick={() => {
                            const orderToPrint = kitchenOrders.find((order) =>
                              order.items.some(
                            ...
                            if (orderToPrint) {
                              ...
                              const itemHtml = orderToPrint.items
                                .map(
                                  (i) =>
                                    `<li>${i.item_title}${
                                      i.variant_title
        ...
        <html><head><title>Receipt - Order #${orderToPrint.id}</title></head>
        <body>
          <h2>Order Receipt</h2>
          <p><strong>Order ID:</strong> ${orderToPrint.id}</p>
          <p><strong>Token No:</strong> ${orderToPrint.token_no || "-"}</p>
          <ul>${itemHtml}</ul>
        </body>
        </html>
      `);
