...
export default function InvoicesPage() {
  ...
  const btnViewReceipt = async (orderIdsArr, tokens, payment_type_id) => {
    try {
      toast.loading("Please wait...");
  ...
  const btnPrintReceipt = async (orderIdsArr, tokens, payment_type_id) => {
    try {
      toast.loading("Please wait...");
      const res = await getInvoiceOrders(orderIdsArr);
      toast.dismiss();
      ...
      if (res.status == 200) {
        const {
          ...
          total,
          orders: ordersArr,
          invoice_id, // assuming it's returned from backend
        } = res.data;
        ...
        setDetailsForReceiptPrint({
          ...
          taxTotal: taxTotal,
          payableTotal: total,
          tokenNo: token_no || tokens,
          orderId: orderIds,
          date: date,
          paymentMethod: paymentMethodText,
          invoiceNumber: invoice_id || orderIds, // fallback if no invoice_id
        });
                ...
                return (
                  <tr key={index}>
                    ...
                    <td className="flex items-center gap-2">
                      <button
                        onClick={() => {
                          btnViewReceipt(orderIdsArr, tokens, payment_type_id);
                        }}
                        className="btn btn-sm btn-circle text-slate-500"
                      >
                        <IconReceipt stroke={iconStroke} />
                      ...
                      <button
                        onClick={() => {
                          btnPrintReceipt(orderIdsArr, tokens, payment_type_id);
                        }}
                        className="btn btn-sm btn-circle text-slate-500"
                      >
                        <IconPrinter stroke={iconStroke} />
