...
import { VITE_BACKEND_SOCKET_IO, iconStroke, API } from "../config/config";
import {
  cancelAllQROrders,
  cancelQROrder,
  createOrder,
...
import { createStripeCheckoutSession } from "../controllers/payment.controller";
...
export default function POSPage() {
  ...
  useEffect(() => {
    if (!window.StripeTerminal) return; // SDK script not loaded yet
    ...
    return () => {
      /* no explicit stop method in browser SDK */
    };
  }, []);

  // Handle when a reader is detected
  const handleReaderDetected = (reader) => {
    console.log("Reader detected:", reader);
    setReader(reader);
  };

  // Handle when a reader is disconnected
  const handleReaderDisconnected = (reader) => {
    console.log("Reader disconnected:", reader);
    setReader(null);
  };

  // Handle when a payment method is received
  const handlePaymentMethodReceived = async (paymentMethod) => {
    console.log("Payment method received:", paymentMethod);
    const { paymentIntent, error } = await createPaymentIntent(paymentMethod);
  ...
  // Create a Payment Intent (Backend call required)
  const createPaymentIntent = async () => {
    ...
    const { paymentIntent, error: collectErr } =
      await terminal.collectPaymentMethod(clientSecret);
    if (collectErr) {
      console.error("collectPaymentMethod error:", collectErr);
      toast.error(collectErr.message || "Failed to collect payment");
      setPaymentStatus("failed");
      throw collectErr;
    }
  ...
  const confirmPayment = async (paymentIntent) => {
    const { paymentIntent: processed, error: processErr } =
      await terminal.processPayment(paymentIntent);
    if (processErr) {
      console.error("processPayment error:", processErr);
      toast.error(processErr.message || "Payment failed");
      setPaymentStatus("failed");
      return null;
    }
    handlePaymentSucceeded(processed);
    return processed;
  };
  ...
  const connectReader = async () => {
    if (!terminal) return;
    try {
      // Use simulated readers in dev if needed: { simulated: true }
      const { discoveredReaders, error } = await terminal.discoverReaders({
        simulated: true,
      });
      if (error) {
        console.error("discoverReaders error:", error);
        return toast.error(error.message || "Failed to discover readers");
      }
      if (!discoveredReaders?.length) {
        return toast.error("No readers found. Please connect a reader.");
      }
      const selected = discoveredReaders[0];
      const { reader: connected, error: connErr } =
        await terminal.connectReader(selected);
      if (connErr) {
        console.error("connectReader error:", connErr);
        return toast.error(connErr.message || "Failed to connect to reader");
      }
      setReader(connected);
      toast.success(`Connected to ${connected.label || connected.device_type}`);
    } catch (e) {
      console.error("Error connecting to reader:", e);
  ...
  const handleDiningOptionChange = (e) => {
    setDiningOption(e.target.value); // Update the dining option state
  };
    ...
    return (
      <Page className="px-4 py-3 flex flex-col min-h-0">
        <div className="mt-4 h-[calc(100vh-136px)] flex gap-4 skeleton">
          <div className="border border-salespos-border-green-light rounded-2xl h-full w-[70%] overflow-y-auto"></div>
          <div className="border border-salespos-border-green-light rounded-2xl h-full w-[30%] relative flex flex-col"></div>
        </div>
      </Page>
    );
  }
  ...
  const btnClearQROrders = async () => {
    ...
    try {
      ...
      const message =
        ...
        "We're getting issue while clearing orders! Please try later!";
      console.log(error);
      toast.dismiss();
      toast.error(message);
    }
  ...
  const calculateOrderSummary = () => {
    ...
    cartItems.forEach((item) => {
      ...
      const tax_discount = item.tax_discount;
      console.log(tax_discount);
      ...
      if (taxType == "exclusive") {
        ...
        const discount = (itemPrice * tax_discount) / 100;
        total_discount += discount;
        const price_after_discount = priceWithTax - discount;

        taxTotal += tax;
        itemsTotal += itemPrice;
        payableTotal += price_after_discount;
      } else if (taxType == "inclusive") {
        const base_price = itemPrice * (100 / (100 + taxRate));
        const tax = itemPrice - base_price;
        const discount = base_price * (tax_discount / 100);
        const priceWithoutTax = itemPrice - tax;
        total_discount += discount;

        taxTotal += tax;
        itemsTotal += priceWithoutTax;
        payableTotal += itemPrice - discount;
      } else {
        itemsTotal += itemPrice;
        payableTotal += itemPrice;
        taxTotal = 0;

        total_discount += (itemPrice * tax_discount) / 100;
      }
    });
  ...
  const btnShowPayAndSendToKitchenModal = () => {
    ...
    const { itemsTotal, taxTotal, payableTotal, total_discount } =
  ...
  const handleStripePayment = async () => {
    console.log("=== STRIPE PAYMENT FLOW STARTED ===");
    try {
      ...
      console.log("Input validation - Delivery type:", deliveryType);
      console.log("Input validation - Table ID:", tableId);
      console.log("Input validation - Customer type:", customerType);
      console.log("Input validation - Customer:", customer);

      // Check for required dining options
      if (!deliveryType) {
        console.error("Missing delivery type");
        toast.error("Please select delivery type!");
        return;
      }

      if (deliveryType === "dinein" && !tableId) {
        console.error("Missing table selection for dine-in");
        toast.error("Please select a table!");
        return;
      }

      // First log the payment type to check if it's coming through correctly
      console.log("Selected payment type ID:", state.selectedPaymentType);
      console.log("Payment types available:", paymentTypes);

      // Find the Stripe payment type in the available payment types
      const stripePaymentType = paymentTypes.find(
        (type) => type.icon === "stripe"
      );

      if (stripePaymentType) {
        console.log("Found Stripe payment type:", stripePaymentType);
      } else {
        console.warn(
          "Stripe payment type not found in available payment types"
        );
      }

      toast.loading("Preparing Stripe payment...");
      ...
      console.log("Creating order for Stripe payment with data:", orderData);

      try {
        console.log("Calling createOrderForStripePayment API");
        const orderResponse = await createOrderForStripePayment(
          cartItems,
          deliveryType,
          customerType,
          customer,
        ...
        console.log("Current origin for return URLs:", origin);
        ...
        try {
          const stripeData = {
            ...
            orderId,
            currency: currency === "$" ? "USD" : "GBP", // Adjust based on your currency
            customerEmail: customer?.email || "customer@example.com", // Add fallback email
            successUrl: `${origin}/success?session_id={CHECKOUT_SESSION_ID}`,
            cancelUrl: `${origin}/cancelled-payment`,
          };

          console.log("Stripe session data:", stripeData);
          const stripeResponse = await createStripeCheckoutSession(stripeData);

          console.log("Stripe API response:", stripeResponse);

          toast.dismiss();

          if (stripeResponse.status === 200 && stripeResponse.data.url) {
            console.log("Redirecting to Stripe:", stripeResponse.data.url);

            // Redirect to Stripe checkout - force redirection to ensure it happens
            console.log("Attempting window.location.replace redirect");
            window.location.replace(stripeResponse.data.url);
            console.log("Redirect instruction sent to browser");
          } else {
            console.error("Failed to create Stripe session:", stripeResponse);
            toast.error("Error creating payment session");
          }
        } catch (stripeError) {
          console.error("Stripe API Error:", stripeError);
          const errorDetails = stripeError.response
            ? stripeError.response.data
            : stripeError.message;
          console.error("Stripe API Error Details:", errorDetails);
          throw stripeError;
        }
      } catch (orderError) {
        console.error("Order Creation Error:", orderError);
        const errorDetails = orderError.response
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        if (state.selectedQrOrderItem) {
          ...
          newQROrders = state?.qrOrders?.filter(
            (item) => item.id != state.selectedQrOrderItem
          );
        }
  ...
  const setCustomer = (customer) => {
    ...
    if (customer) {
      setState({
        ...state,
        customer: { name: customer.label, id: customer.value },
        customerType: "CUSTOMER",
      });
      document.getElementById("modal-search-customer").close();
    } else {
  ...
  return (
    <Page className="px-4 py-3 flex flex-col min-h-0">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          >
            ...
            {state.qrOrdersCount > 0 && (
              <div className="absolute -top-2 -right-2 w-4 h-4 rounded-full bg-red-500 text-white text-xs flex items-center justify-center">
                {state.qrOrdersCount}
              </div>
            )}
          </button>
      ...
      <div className="mt-4 md:h-[calc(100vh-136px)] flex flex-col-reverse md:flex-row gap-4">
        {/* pos items */}
        <div className="border border-salespos-border-green-light rounded-2xl h-full md:w-[70%] overflow-y-auto">
          {/* categories, search */}
          ...
          <div className="grid grid-cols-2 gap-4 w-full z-0 px-4 pb-4 rounded-b-2xl">
            {menuItems
              ...
              .map((menuItem, i) => {
                ...
                return (
                  <div
                    className="bg-white border border-salespos-border-green-light rounded-2xl p-2 flex gap-2"
                    key={i}
                  >
                    ...
                    <div>
                      ...
                      <div className="mt-2">
                        <button
                          ...
                          className="rounded-full px-4 py-1 bg-salespos-green hover:bg-salespos-green-dark transition active:scale-95 text-white flex items-center justify-center"
                        >
                          Add
                        </button>
                      </div>
        ...
        {/* cart */}
        <div className="border border-salespos-border-green-light rounded-2xl h-full md:w-[30%] relative flex flex-col">
          <div className="sticky w-full px-4 py-4 bg-gradient-to-b from-white to-white/0 border-b rounded-t-2xl">
            ...
            >
              <input
                ...
                className="cursor-pointer text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light"
                onFocus={btnOpenSearchCustomerModal} // optional: keyboard access
              />
            ...
            <select
              ...
              className="mt-3 text-sm text-gray-500 w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light"
            >
              ...
              {storeTables.map((table, index) => {
                return (
                  <option
                    value={table.id}
                    key={index}
                    disabled={table.occupancy_status === "occupied"}
                    className={
          ...
          <div className="flex-1 flex flex-col gap-4 overflow-y-auto px-4 pb-36">
            ...
            {cartItems?.map((cartItem, i) => {
              ...
              return (
                <div
                  key={i}
                  className="text-sm border border-salespos-border-green-light rounded-lg p-2 relative"
                >
                  <p>
          ...
          <div className="absolute w-full bottom-0 pb-4 rounded-b-2xl px-4 bg-white/20 backdrop-blur border-t">
            ...
            <div className="mt-2">
              <button
                onClick={btnShowPayAndSendToKitchenModal}
                className="text-sm rounded-lg border border-transparent bg-salespos-green text-white hover:bg-salespos-green-dark transition active:scale-95 hover:shadow-lg px-2 py-1 w-full flex items-center justify-center gap-1"
              >
                <div>
                ...
                >
                  ...
                  <div className="flex flex-col gap-1">
                    <button
                      ...
                      className="rounded-full transition active:scale-95 text-salespos-green w-6 h-6 bg-green-50 hover:bg-salespos-green-dark hover:text-white flex items-center justify-center"
                    >
        ...
        <div className="modal-box h-96">
          <div className="flex items-center justify-between">
            ...
            <form method="dialog">
              <button
                ...
                className="btn btn-circle btn-sm bg-gray-50 hover:bg-gray-100 text-gray-500 border-none"
              >
          ...
          <div className="my-4 flex items-end gap-2">
            <div className="flex-1">
              ...
              {/* <input ref={searchCustomerRef} type="search" name="searchCustomerRef" id='searchCustomerRef' className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Customer Phone here..." /> */}
      ...
      >
        <div className="modal-box">
          <div className="flex items-center justify-between">
            ...
            {/* No form for this close button */}
            <button
              onClick={() =>
                document
                  .getElementById("modal-pay-and-send-kitchen-summary")
                  .close()
              }
              className="hover:bg-red-100 border-none transition active:scale-95 bg-red-50 text-red-500 btn btn-sm btn-circle"
            >
              <IconX size={18} stroke={iconStroke} />
            </button>
          </div>
          ...
          <div className="my-6 flex items-center justify-center divide-x w-full">
            ...
            <div className="flex-1 text-center">
              <p>Payable Total</p>
              <p className="text-lg font-bold text-salespos-green">
                {currency}
            ...
            <div className="flex-1 text-center">
              ...
              <p className="text-lg font-bold text-salespos-green">
                {currency}
                {state.total_discount.toFixed(2)}
              </p>
            </div>
          </div>
                ...
                <label key={i} className="">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      console.log("Payment type icon:", paymentType.icon);
                      console.log("Payment type ID:", paymentType.id);
                      console.log(
                        "Payment type ID type:",
                        typeof paymentType.id
                      );
                      setState({
                        ...state,
                  ...
                  <label
                    ...
                    className="border rounded-2xl flex items-center justify-center gap-1 flex-col px-4 py-3 text-gray-500 peer-checked:border-salespos-green peer-checked:text-salespos-green peer-checked:font-bold cursor-pointer transition"
                  >
                    {paymentType?.icon ? (
          ...
          <div className="modal-action flex flex-col items-center justify-center w-full gap-3">
            {/* DEBUG: Check payment types and conditions */}
            {console.log("All payment types:", paymentTypes)}
            {console.log("Selected payment type:", state.selectedPaymentType)}
            {console.log(
              "Card payment condition:",
              paymentTypes.some(
                (type) =>
                  (type.icon === "stripe" || type.icon === "card") &&
                  String(state.selectedPaymentType) === String(type.id)
              )
            )}
            ...
            {paymentTypes.some(
              ...
              <div className="w-full">
                {/* Stripe Terminal Reader Connect UI */}
                <StripeReaderConnect />

                {/* Proceed Button after connection */}
                {window.connectedStripeReader && (
                  <button
                    type="button"
                    onClick={() => {
                      console.log("Pay with Terminal clicked");
                      document
                        .getElementById("modal-pay-and-send-kitchen-summary")
                        .close();
                      window.location.href = "/terminal-payment";
                    }}
                    className="mt-4 w-full text-center rounded-lg hover:bg-orange-600 transition active:scale-95 hover:shadow-lg px-4 py-4 bg-orange-500 text-white font-bold text-lg"
                  >
                    PAY WITH TERMINAL
                  </button>
                )}
            ...
            {/* CARD PAYMENT BUTTON - Only shown when card/stripe payment type is selected */}
            {paymentTypes.some(
              (type) =>
                (type.icon === "stripe" || type.icon === "card") &&
                String(state.selectedPaymentType) === String(type.id)
              ...
              <button
                ...
                onClick={() => {
                  console.log(
                    "Pay with Card button clicked with payment type:",
                ...
                className="w-full text-center rounded-lg hover:bg-blue-600 transition active:scale-95 hover:shadow-lg px-4 py-4 bg-blue-500 text-white font-bold text-lg"
              >
                PAY WITH CARD (STRIPE)
              </button>
            )}

            {/* Default fallback: Send to Kitchen */}
            {!paymentTypes.some(
              (type) =>
                ["stripe", "card", "terminal"].includes(type.icon) &&
                String(state.selectedPaymentType) === String(type.id)
            ) && (
              <button
                type="button"
                onClick={() => {
                  document
                    .getElementById("modal-pay-and-send-kitchen-summary")
                    .close();
                  setTimeout(() => btnPayAndSendToKitchen(), 100);
                }}
                className="w-full text-center rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white"
              >
                Send to Kitchen
              </button>
            )}
            ...
            <button
              ...
              className="w-full mt-2 text-center rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-2 bg-gray-100 text-gray-600"
            >
              Cancel
            </button>
          </div>
      ...
      >
        <div className="modal-box">
          ...
          <div className="my-8 mx-auto w-fit">
            <div className="flex items-center gap-2">
              <div className="w-12 h-12 flex items-center justify-center bg-salespos-green text-white rounded-full">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-1 text-xs">
                      ...
                      <p className="">
                        {customer_type == "WALKIN" ? "WALKIN" : customer_name}
                      </p>
                    </div>
