...
export default function CustomersPage() {
  ...
  const btnShowUpdate = (id, phone, name, email, birthDate, gender) => {
    customerIdRef.current.value = id;
    phoneRef.current.value = phone || "";
    nameRef.current.value = name || "";
    emailRef.current.value = email || "";

    const bDate = birthDate ? new Date(birthDate) : null;
    birthDateRef.current.value = bDate
      ? `${bDate.getFullYear()}-${(bDate.getMonth() + 1)
          .toString()
          .padStart(2, "0")}-${bDate.getDate().toString().padStart(2, "0")}`
      : "";
  ...
  async function btnUpdate() {
    const id = customerIdRef.current.value;
    ...
    if (!id || !phone || !name) {
      toast.error("Customer ID, phone, and name are required!");
      return;
    }
    ...
    <Page>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
          {customers.map((customer, index) => {
            ...
            return (
              <div
                key={phone}
                className="border border-salespos-border-green-light rounded-2xl px-4 py-5"
              >
      ...
      >
        <div className="modal-box">
          ...
          <input type="hidden" ref={customerIdRef} />
          ...
          <div className="mt-4">
            ...
            <input
              ...
              disabled={phoneRef.current?.value?.length > 0}
            />
          </div>
