...
export default function ReservationPage() {
  ...
  useEffect(() => {
    if (!isLoading && Array.isArray(reservations)) {
      console.log("📥 Current reservations fetched from API:", reservations);
    } else if (error) {
      console.error("❌ Error fetching reservations:", error);
    }
  }, [isLoading, reservations, error]);
  ...
  const fetchUnavailableTables = async (selectedDateTime) => {
    try {
      const response = await authFetch(buildApiUrl("/reservations?type=all"));
      const allReservations = await response.json();

      const start = new Date(selectedDateTime);
      const blockMinutes = parseInt(
        import.meta.env.VITE_RESERVATION_BLOCK_DURATION_MINUTES || "120"
      );
      ...
      const conflicts = allReservations.filter((r) => {
        ...
        const resEnd = new Date(resStart.getTime() + blockMinutes * 60000);
      ...
      const tableIds = conflicts.map((r) => r.table_id);
      setUnavailableTableIds(tableIds);
    } catch (err) {
      console.error("❌ Error checking unavailable tables:", err);
    }
  };

  const resetAddReservationForm = () => {
    reservationDateRef.current.value = null;
    reservationTableRef.current.value = "";
    reservationPeopleCountRef.current.value = null;
    reservationStatusRef.current.value = "booked";
    reservationNotesRef.current.value = "";
    clearSelectedCustomer();
    setUnavailableTableIds([]); // reset unavailable table cache
  };
  ...
  const btnAdd = async () => {
    ...
    if (!peopleCount) {
      toast.error("Please provide people count!");
      return;
    }

    try {
      toast.loading("Adding reservation...");
      ...
      console.log("✅ Reservation Add Response:", res);
      ...
      if (res.status === 200) {
        ...
        clearSelectedCustomer();
        toast.dismiss();
        toast.success(`${res.data.message} (Code: ${res.data.uniqueCode})`);
      }
    } catch (error) {
      console.error("❌ Reservation Add Error:", error);
      const message = error?.response?.data?.message || "Something went wrong!";
      toast.dismiss();
      toast.error(message);
    }
  ...
  const btnShowUpdate = (id, date, table, peopleCount, status, notes) => {
    ...
    const localISOTime = new Date(
      ...
      .toISOString()
      .slice(0, 16);
    updateReservationDateRef.current.min = localISOTime;
  ...
  const btnUpdate = async () => {
    ...
    const rawDate = updateReservationDateRef.current.value;
    const date = rawDate;
    ...
    <Page>
      <div className="flex flex-wrap gap-4 flex-col md:flex-row md:items-center md:justify-between">
          ...
          {isManageAllowed && (
            <button
              onClick={() => {
                ...
                reservationDateRef.current.min = localISOTime;

                resetAddReservationForm(); // 👈 clean everything
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={btnSearch}
            className="text-white bg-salespos-green transition hover:bg-salespos-green/80 active:scale-95 rounded-lg px-4 py-1 outline-salespos-border-green-light"
          >
            Search
          </button>
          <button
        ...
        <div className="mt-6">
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
            {state.searchResults.map((reservation, index) => {
              ...
              const dateFormatted = `${d.getFullYear()}-${(d.getMonth() + 1)
                .toString()
                .padStart(2, "0")}-${d
                .getDate()
                .toString()
            ...
            <div className="flex gap-2 mt-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  type="date"
                  ref={fromDateRef}
                  className="text-sm w-full border rounded-lg px-4 py-1 bg-gray-50 outline-salespos-border-green-light"
                />
              </div>
              <div className="flex-1">
                <label
        ...
        <div className="modal-box">
          ...
          <div className="mt-4">
            ...
            <AsyncCreatableSelect
              value={state.customer} // ✅ Controlled input
              menuPlacement="auto"
              loadOptions={searchCustomersAsync}
              isClearable
              noOptionsMessage={() => "Type something to find..."}
              onChange={(v) => {
                setCustomer(v);
              }}
          ...
          <div className="flex gap-4 w-full my-4">
            <div className="flex-1">
              ...
              <input
                ...
                onChange={(e) => fetchUnavailableTables(e.target.value)}
            ...
            <div className="flex-1">
              ...
              >
                ...
                {storeTables.map((storeTable) => {
                  const isUnavailable = unavailableTableIds.includes(
                    storeTable.id
                  );
              ...
              {unavailableTableIds.length === storeTables.length && (
          ...
          <div className="my-4">
            ...
            <textarea
              ...
              className="w-full block h-24 text-sm border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light"
            ></textarea>
          </div>
          ...
          <div className="flex gap-4 w-full my-4">
            ...
            <div className="flex-1">
              ...
              >
                ...
                {storeTables
                  .filter(
                    (storeTable) => storeTable.occupancy_status === "vacant"
                  )
                  .map((storeTable, index) => (
            ...
            <form method="dialog">
              ...
              <button
                onClick={() => {
                  updateReservationIdRef.current.value = null;
                  updateReservationDateRef.current.value = null;
                  updateReservationTableRef.current.value = "";
                  updateReservationPeopleCountRef.current.value = null;
                  updateReservationStatusRef.current.value = "booked";
                  updateReservationNotesRef.current.value = "";
                }}
                className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500"
              >
                Close
