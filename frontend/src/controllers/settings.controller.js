...
export function getStoreTableDetails() {
  const APIURL = `/settings/store-table-details`;
  const { data, error, isLoading } = useSWR(APIURL, fetcher);
  return {
    data,
    error,
...
// export async function getStoreTableDetails() {
//   try {
//     const response = await axios.get(`${API}/settings/store-table-details`);
...
export async function addNewStoreTable(title, floor, seatingCapacity, occupancyStatus) {
  try {
    const response = await ApiClient.post("/settings/store-tables/add", {
      title, floor, seatingCapacity, occupancyStatus
    })
    return response;
  } catch (error) {
    throw error;
...
export async function updateStoreTable(id, title, floor, seatingCapacity, occupancyStatus) {
  try {
