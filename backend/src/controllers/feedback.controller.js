...
exports.addFeedback = async (req, res) => {
    try {
        ...
        const { review, rating, customer_name, service, food } = req.body;

        var review_result = '';
        if (!(rating && customer_name)) {
            return res.status(400).json({
                success: false,
                message: "Please provide required details: rating, customer_name"
            });
        }
        if (rating > 3) {
        ...
        const feedbackId = await addCustomerReviewDB(review, rating, customer_name, review_result,service, food, tenantId);
