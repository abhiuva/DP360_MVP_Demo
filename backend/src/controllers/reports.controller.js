const {
  getOrdersCountDB,
  getNewCustomerCountDB,
  getRepeatCustomerCountDB,
  getAverageOrderValueDB,
  getTotalCustomersDB,
  getTotalNetRevenueDB,
  getTotalTaxDB,
  getRevenueDB,
  getTopSellingItemsDB,
  getTotalPaymentsByPaymentTypesDB,
  getItemSaleReportDB,
  getCustomerDueReportDB,
  getSupplierDueReportDB,
  getEmployeesDB,
  getAttendanceReportDB,
  getExpenseReportBD,
  getSalesReportDB,
  getStockAlertReportDB,
  getPurchaseReportDB,
  getWorkPeriodReportDB,
  getSaleSummaryReportDB,
  getSaleDetailedReportDB,
...
exports.getAttendanceReport = async (req, res) => {
  try {
    ...
    const attendanceReport = await getAttendanceReportDB(
      employeeId,
      startDate,
      endDate
    );
...
exports.getStockAlertReport = async (req, res) => {
  try {
    const tenantId = req.user?.tenant_id ?? req.query?.tenant_id ?? null;
    if (!tenantId) {
      return res
        .status(400)
        .json({ success: false, message: "Tenant not resolved" });
    }
    const reportData = await getStockAlertReportDB(tenantId);
...
// PURCHASE REPORT: Do NOT require tenant here (schema supplier_orders has no tenant_id)
exports.getPurchaseReport = async (req, res) => {
  try {
    // Read but DO NOT enforce; service ignores because table has no tenant_id
    const _tenantId = req.user?.tenant_id ?? req.query?.tenant_id ?? null;
    ...
    const reportData = await getPurchaseReportDB(p, startDate, endDate); // no tenantId param
    return res.status(200).json({ success: true, data: reportData });
  } catch (error) {
