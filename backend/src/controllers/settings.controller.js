...
exports.addStoreTable = async (req, res) => {
  try {
    ...
    const occupancyStatus = req.body.occupancyStatus;
    ...
    const id = await addStoreTableDB(
      title,
      floor,
      seatingCapacity,
      occupancyStatus,
...
exports.getStoreTableDetails = async (req, res) => {
  try {
    const tenantId = req.user.tenant_id;

    const result = await getStoreTableDetailsDB(tenantId);
    return res.status(200).json(result);
  } catch (error) {
    console.error(error);
    return res.status(500).json({
...
// exports.getStoreTableDetails = async (req, res) => {
//     try {
//         const result = await getStoreTableDetailsDB();
...
exports.updateStoreTable = async (req, res) => {
  try {
    ...
    console.error("Error in updateStoreTable:", error);
    return res.status(500).json({
      success: false,
      message: "Something went wrong! Please try later!",
    });
...
exports.getPublicStoreDetails = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, data: storeSettings });
  } catch (error) {
    console.error(error);
