...
exports.doCustomerExistDB = async (email, tenantId) => {
    const conn = await getMySqlPromiseConnection();
    try {

        const sql = `
        SELECT phone, email, name FROM customers
...
exports.getCustomersDB = async(page, perPage, sort, filter, tenantId) => {
    ...
    try {
        ...
        const [customers] = await conn.execute(
            `SELECT id, phone, name, email, birth_date, gender, is_member, created_at FROM customers ${filterQuery} ${sortedBy} LIMIT ${limit} OFFSET ${offset} ;`
        );
...
exports.getCustomerDB = async(id, tenantId) => {
    const conn = await getMySqlPromiseConnection();
    try {

        const [result] = await conn.execute(
            `SELECT id, phone, name, email, birth_date, gender, is_member, created_at FROM customers
...
exports.updateCustomerDB = async (id, phone, name, email, birthDate, gender, tenantId) => {
    const conn = await getMySqlPromiseConnection();
    try {
        ...
        UPDATE customers
        SET
        name = ?, email = ?, birth_date = ?, gender = ?, phone = ?
        WHERE id = ? AND tenant_id = ?
        `;

        await conn.query(sql, [name, email, birthDate, gender, phone, id, tenantId]);
        return;
    } catch (error) {
        console.error(error);
        throw error;
...
exports.deleteCustomerDB = async (id, tenantId) => {
    const conn = await getMySqlPromiseConnection();
    try {
