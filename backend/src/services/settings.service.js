...
exports.addStoreTableDB = async (title, floor, seatingCapacity, occupancyStatus, tenantId) => {
    const conn = await getMySqlPromiseConnection();

    try {
        const sql = `
        INSERT INTO store_tables
        (table_title, floor, seating_capacity, occupancy_status, tenant_id)
        VALUES (?, ?, ?, ?, ?);
        `;

        const [result] = await conn.query(sql, [title, floor, seatingCapacity,occupancyStatus, tenantId]);
        return result.insertId;
    } catch (error) {
        console.error(error);
        throw error;
...
exports.getStoreTablesDB = async (tenantId) => {
    ...
    try {
        ...
        occupancy_status
        FROM store_tables
        WHERE tenant_id = ? AND occupancy_status IN ("occupied","vacant") ;
        `;

        const [result] = await conn.query(sql, [CONFIG.ENCRYPTION_KEY, tenantId, ["occupied","vacant"]]);
        return result;
    } catch (error) {
        console.error(error);
        throw error;
...
exports.getStoreTableDetailsDB = async (tenantId) => {
  const conn = await getMySqlPromiseConnection();
...
exports.updateStoreTableDB = async (id, title, floor, seatingCapacity, occupancyStatus, tenantId) => {
    const conn = await getMySqlPromiseConnection();

    try {
        const sql = `
        UPDATE store_tables SET
        table_title = ?, floor = ?, seating_capacity = ?, occupancy_status = ?
        WHERE id = ? AND tenant_id = ?;
        `;

        await conn.query(sql, [title, floor, seatingCapacity, occupancyStatus, id, tenantId]);
        return;
    } catch (error) {
        console.error(error);
        throw error;
...
exports.updateTableOccupancyDB = async (id, occupancyStatus, tenantId) => {
    const conn = await getMySqlPromiseConnection();
    ...
    try {
        ...
        await conn.query(sql, [occupancyStatus, id, tenantId]);
        return;
    } catch (error) {
        console.error(error);
        throw error;
