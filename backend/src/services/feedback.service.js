...
exports.addCustomerReviewDB = async (review, rating, customer_name, review_result,service, food, tenant_id) => {
    const conn = await getMySqlPromiseConnection();
    try {

        const sql = `
        INSERT INTO customer_feedbacks
        (review, rating, customer_name, review_result, service, food, tenant_id)
        VALUES
        (?, ?, ?, ?, ?,?,?);
        `;

        const [result] = await conn.query(sql, [review, rating, customer_name, review_result, service, food,tenant_id]);
