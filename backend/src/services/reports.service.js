...
AND ea.attendanceDate BETWEEN CONCAT(?, ' 00:00:00') AND CONCAT(?, ' 23:59:59');
    `;
  const [attendanceReport] = await conn.execute(query, [
    employeeId,
    startDate,
    endDate,
  ]);
...
exports.getStockAlertReportDB = async (tenantId) => {
  const conn = await getMySqlPromiseConnection();
  try {
    const sql = `
  ...
  try {
    ...
    switch (period) {
      case "daily":
        query = `
          SELECT 
            ...
            SUM(CASE WHEN so.payment_status = 'pending' THEN (si.quantity * si.price) - COALESCE(so.amount_paid,0) ELSE 0 END) AS due_amount,
            SUM(CASE 
              WHEN so.payment_status = 'pending' THEN COALESCE(so.amount_paid,0)
              WHEN so.payment_status = 'paid'    THEN (si.quantity * si.price)
              ELSE 0 
            END) AS paid_amount,
            SUM(si.quantity * si.price) AS total_amount
          FROM supplier_orders so
          JOIN supplier_items si ON si.order_id = so.id
      ...
      case "weekly":
        query = `
          SELECT
            DATE_FORMAT(so.date, '%x-%v') AS order_date,
            SUM(CASE WHEN so.payment_status = 'pending' THEN (si.quantity * si.price) - COALESCE(so.amount_paid,0) ELSE 0 END) AS due_amount,
            SUM(CASE 
              WHEN so.payment_status = 'pending' THEN COALESCE(so.amount_paid,0)
              WHEN so.payment_status = 'paid'    THEN (si.quantity * si.price)
...
exports.getSaleDetailedReportDB = async (startDate, endDate) => {
  ...
  try {
	...
	o.invoice_id,
        ...
        RIGHT JOIN payment_types pt ON inv.payment_type_id = pt.id
        WHERE o.date BETWEEN CONCAT(?, ' 00:00:00') AND CONCAT(?, ' 23:59:59');`;
