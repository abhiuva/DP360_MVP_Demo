...
exports.addSupplierItemDB = async (title, price, netPrice, taxId, discount, categoryId, tenantId, inventoryId) => {
    const conn = await getMySqlPromiseConnection();
    try {

        const sql = `
        INSERT INTO supplier_items
        (title, price, net_price, tax_id, discount, category, tenant_id, inventory_id)
        VALUES
        (?, ?, ?, ?, ?, ?, ?, ?);
        `;

        const [result] = await conn.query(sql, [title, price, netPrice, taxId, discount, categoryId, tenantId, inventoryId]);
...
exports.updateMenuItemDB = async (id, title, price, netPrice, taxId, discount, categoryId, tenantId, inventoryId) => {
    const conn = await getMySqlPromiseConnection();
    try {

        const sql = `
        UPDATE menu_items SET
        title = ?, price = ?, net_price = ?, tax_id = ?,discount = ?, category = ?, inventory_id = ?
        WHERE id = ? AND tenant_id = ?;
        `;

        await conn.query(sql, [title, price, netPrice, taxId, discount, categoryId, inventoryId, id, tenantId]);
...
exports.getAllMenuItemsDB = async (tenantId) => {
    ...
    try {
        ...
        const sql = `
        SELECT 
        i.id, i.title, i.discount AS tax_discount, price, net_price, tax_id, t.title AS tax_title, t.rate AS tax_rate, t.type AS tax_type, i.category as category_id, c.title AS category_title, i.image, inventory_id, inv.title as inventory_title
        FROM menu_items i
        LEFT JOIN taxes t
        ON i.tax_id = t.id
